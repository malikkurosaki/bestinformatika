              ...
              children: [
                ...
                Flexible(
                  child: PageView.builder(
                    ...
                    itemBuilder: (context, pekan) => 
                    Column(
                      children: [
                        // tanggalan tanggal angka [ hari ]
                        Container(
                            ...
                            children: [
                              ...
                              Expanded(
                                ...
                                GridView.count(
                                  ...
                                  physics: ScrollPhysics(),
                                  crossAxisCount: 7,
                                  children: List.generate(CalendarCtrl.kalender[pekan].length, (i1) => 
                                    Center(
                                      child: Text(CalendarCtrl.kalender[pekan][i1]['value'].toString(),
                                        style: TextStyle(
                                          fontWeight: CalendarCtrl.kalender[pekan][i1]['type'] != 1?FontWeight.normal: FontWeight.w700,
                                          color: CalendarCtrl.kalender[pekan][i1]['type'] != 1?Colors.blueGrey[700]: Colors.black
                                        ),
                                      ),
                                    )
                                  )
                                              ...
                                              child: Row(
                                                children: [
                                                  Container(
                                                    ...
                                                    width: lebarKiri,
                                                    child: Text(CalendarCtrl.lsRoomType[i2]['rooms'][i3]['no_room'])
                                                  ),
                                                  Expanded(
                                                    child: Stack(
                                                      children: [
                                                        ...
                                                        CalendarCtrl.lsRoomType[i2]['rooms'][i3]['events'].length == 0? SizedBox.shrink(): 
                                                        Column(
                                                          children: List.generate(CalendarCtrl.lsRoomType[i2]['rooms'][i3]['events'].length, (i4) => 
                                                            Row(
                                                              children: List.generate(CalendarCtrl.lsRoomType[i2]['rooms'][i3]['events'][i4]['event_date_rage'].length, (i5) => 
                                                                CalendarCtrl.lsRoomType[i2]['rooms'][i3]['events'][i4]['event_date_rage'][i5]['pekan'] != pekan?SizedBox.shrink():
                                                                Text("ini event")
                                                              ),
                                                            )
                                                          ),
...
class CalendarCtrl extends PhisCtrl{
  ...
  static iniAssyncRoomType() async {
   try {
      ...
      Response res2 = await ControllerApi.to.calendarBooking("2021-01-01", "2021-01-31");

      final List data = res.body['data']['data'];
      final List data2 = res2.body['data']['data'];
      
      final olah = data.map((e) => {
        "kd_jenis": e["kd_jenis"],
        "nm_jenis": e['nm_jenis'],
        "rooms": e['rooms'].map( (e1) => {
          "no_room": e1['no_room'],
          "nama_room": e1['nama_room'],
          "events": []
        }).toList()

      }).toList();

      final olah2 = data2.map((e) => {
        "roomId": e['roomId'],
        ...
        "checkIn": e['checkIn'],
        "checkOut": e['checkOut'],
        "event_date_rage": tanggalRange(e['checkIn'], e['checkOut'])
      }).toList();

      for(var i =0; i < olah.length; i++){
        for(var i2 = 0; i2 < olah[i]['rooms'].length; i2++){
          for(var i3 = 0; i3 < olah2.length; i3++){
            //print([olah[i]['rooms'][i2]['no_room'], olah2[i3]['roomId']]);
            if(olah[i]['rooms'][i2]['no_room'] == olah2[i3]['roomId']){
              olah[i]['rooms'][i2]['events'].add(olah2[i3]);
            }
          }
        }
      }
      
      lsRoomType.assignAll(olah);
      print("assync room type");
   } catch (e) {
     print(e);
  ...
  static tanggalanSebulan(DateTime kal, int date){
  ...
  static munculkanApi(var data) => Get.dialog(SingleChildScrollView(child: Card(child: Text(JsonEncoder.withIndent("    ").convert(data).toString()),),));
  ...
  static List tanggalRange(String checkIn, String checout) => List.generate(DateTime.parse(checout).difference(DateTime.parse(checkIn)).inDays + 1, (index) => {
    ...
    "pekan": getPekan(DateTime(DateTime.parse(checkIn).year, DateTime.parse(checkIn).month, DateTime.parse(checkIn).day + (index)).toString()),
  ...
  static initlistEvent()async{
    Response res = await ControllerApi.to.calendarBooking("2021-01-01", "2021-01-31");
    ...
    //Get.dialog(Card(child: SingleChildScrollView(child: Text(JsonEncoder.withIndent(" ").convert(ijc)),),));
