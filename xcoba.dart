...
import 'lib/rest_restponse_calendar.dart';
import 'lib/rest_response_restaurant.dart';

main() {

  List target = RestRestponseCalendar.listBooking;
...
// List listMap (List target) {
//   final kun = target.map((e) => e.keys.toList()).toList();
//   final val = target.map((e) => e.values.toList()).toList();

//   for (var i = 0; i< kun.length; i++){
//     for(var a = 0; a< kun[i].length; a++){
//       target[i][kun[i][a]] = 
//       (target[i][kun[i][a]] is String)?"":
//       (target[i][kun[i][a]] is int)? 0:
//       (target[i][kun[i][a]] is List)? []:
//       (target[i][kun[i][a]] is Map)? map(target[i][kun[i][a]]):
//       (target[i][kun[i][a]] is bool)? false: "";
//     }
//   }
...
//olah(dynamic ini) => (ini is List)? list(ini): (ini is Map)? map(ini): "bukan list atau map";
...
List ls (List target){
  for(var i = 0; i< target.length; i++){
    ...
    (target[i] is int)? 0:
    (target[i] is List)? []:
    // (target[i] is Map<String, dynamic>)? {}:
    //(target[i] is Map<String, Map<String, dynamic>>)? {"":{}}:
    (target[i] is bool)? false: 
    target[i];
  }
 ...
 mp (dynamic target) {
  // print(target.keys);
  final kun = target.keys.toList();
  for(var i = 0; i< kun.length; i++){
    target[kun[i]] = 
    (target[kun[i]] is String)?"":
    (target[kun[i]] is int)? 0:
    (target[kun[i]] is List)? ls(target[kun[i]]):
    (target[kun[i]] is Map<String, dynamic>)? mp(target[i]):
    (target[kun[i]] is bool)? false: target[kun[i]];
  }
  return target;
}
...
//       },
//       "revenue": {
//         "today": 16666.666666666668,
//         "yesterday": 72765.51111111112
//       },
//       "guest": {
//         "todayAdult": 1,
//         "todayChild": 0,
